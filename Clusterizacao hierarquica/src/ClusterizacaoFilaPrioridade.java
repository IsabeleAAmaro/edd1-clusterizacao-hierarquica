...
public class ClusterizacaoFilaPrioridade {
    ...
    public ClusterizacaoFilaPrioridade(int entradas){
        ...
        while (clusters.size() > 1)
        {
            ...
            clusters.remove(menorDistancia.getC1());
            clusters.remove(menorDistancia.getC2()); 

            /* Remove as distancias dos clusters antigos */
            distancias = removeDistancias(distancias, menorDistancia);
    ...
    public static Cluster criaCluster(int entradas){

        Random random = new Random();

        /* Gera numeros aleatorios pro x e pro y */
        double x = random.nextInt(entradas);
        double y = random.nextInt(entradas);
    ...
    public static HeapBinariaMinima calculaDistancia(List<Cluster> clusters, int tamanho){
        ...
        *  depois do segundo para todos menos o primeiro... */
        ...
        return new HeapBinariaMinima(tamanho, tamanho, vetDistancias);
        
    }

    public static HeapBinariaMinima removeDistancias(HeapBinariaMinima distancias, Distancia menorDistancia){
